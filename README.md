...
Version 0.9 adds a live mission-intelligence strip, persistent local conversation continuity, automatic situational context, current-information research routing, quick commands, and a clearer holographic conversation stream. JARVIS now draws relevant context from projects, tasks, reminders, jobs, approvals, automations, knowledge, memory, and laptop health before answering broad status or priority questions. Use `conversation status` to inspect continuity or `clear conversation` to start fresh.
