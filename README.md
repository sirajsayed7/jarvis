...
For direct remote conversation, configure `GROQ_API_KEY` in the Vercel production environment. For authenticated remote image and camera perception, configure `GEMINI_API_KEY` there as well. The hosted functions validate the Supabase access token and owner email before calling either provider; keys are never sent to the browser.
...
- `SUPABASE_ANON_KEY`
- `JARVIS_OWNER_EMAIL`
- `GROQ_API_KEY` for direct hosted conversation
- `GEMINI_API_KEY` for authenticated hosted visual perception
